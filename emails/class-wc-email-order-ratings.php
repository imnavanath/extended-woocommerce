<?php
/**
 * Class WC_Email_Order_Ratings file.
 *
...
if ( ! class_exists( 'WC_Email_Order_Ratings', false ) ) :
	 ...
	 *
	 * @class       WC_Email_Order_Ratings
	 * @version     1.0.0
	 * @package     WooCommerce/Classes/Emails
	 * @extends     WC_Email
	 */
	class WC_Email_Order_Ratings extends WC_Email {

		/**
		 * Constructor.
		 */
		public function __construct() {
			$this->id             = 'order_rating';
			$this->customer_email = true;
			$this->title          = __( 'Order rating' );
			$this->description    = __( 'Customer order rating emails can be sent to customers containing their order information and payment links.' );
			$this->template_html  = 'emails/order-rating.php';
			$this->template_plain = 'emails/plain/order-rating.php';
		...
		public function get_default_subject( $paid = false ) {
			if ( $paid ) {
				return __( 'Rating for order #{order_number} on {site_title}' );
			} else {
				return __( 'Rating for service from {site_title}' );
			}
		}
		...
		public function get_default_heading( $paid = false ) {
            return __( 'Rating for order #{order_number}' );
		}
		...
		public function get_subject() {
            ...
            if ( $this->object->has_status( array( 'completed', 'processing' ) ) ) {
				...
				return apply_filters( 'woocommerce_email_subject_order_rating_paid', $this->format_string( $subject ), $this->object, $this );
			}

			$subject = $this->get_option( 'subject', $this->get_default_subject() );
			return apply_filters( 'woocommerce_email_subject_order_rating', $this->format_string( $subject ), $this->object, $this );
		}
		...
		public function get_heading() {
			...
			if ( $this->object->has_status( wc_get_is_paid_statuses() ) ) {
				...
				return apply_filters( 'woocommerce_email_heading_order_rating_paid', $this->format_string( $heading ), $this->object, $this );
			}

			$heading = $this->get_option( 'heading', $this->get_default_heading() );
			return apply_filters( 'woocommerce_email_heading_order_rating', $this->format_string( $heading ), $this->object, $this );
		}
		 ...
		 * @return string
		 */
		public function get_thank_you_page_url( $order ) {
		...
		public function get_content_html() {
			return wc_get_template_html(
				...
				array(
					...
					'thank_you_page_url' => $this->object->get_checkout_order_received_url(),
					'sent_to_admin'      => false,
					'plain_text'         => false,
					'email'              => $this,
				)
...
return new WC_Email_Order_Ratings();
